...
`checkpoint-5-bounds-crash.spec.ts`, `checkpoint-5-dependency-boundaries.spec.ts`,
...
`checkpoint-5-photo-evidence.spec.ts`, `photo-capture-coordinator.spec.ts`,
`photography-mission-runtime.spec.ts`, plus HUD/results component specs and the
null presentation adapter.

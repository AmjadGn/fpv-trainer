...
| Starting branch | `feature/v1.3.0-photography-mission-loop` |
| Pre-CP5 engineering tests | 562 |
| Pre-CP5 application CI tests | 606 |
...
Visibility comes only from `MissionSpatialQueryPort`. Unavailable/stale
queries fail with `PHOTO_CAPTURE_SPATIAL_UNAVAILABLE` (infrastructure — does
not count as a scored attempt). Containment uses
`pointInBoundaryShape` from `@fpv/location-validation`.
...
```mermaid
flowchart TB
  Check[Location gen + spatial probe OK?] -->|no| Reject[MISSION_RETRY_RUNTIME_UNAVAILABLE]
  Check -->|yes| Reset[New flight session generation]
  Reset --> Spawn[Reset drone to authored spawn]
  Spawn --> Runtime[photographyRuntime.retry]
  Runtime --> Obj1[Objective 1 / attempts = 1]
```

Location generation unchanged; no definition lookup, asset load, Three group,
or collider reinstall unless infrastructure is invalid. Observer remains
exactly one. Presentation URLs revoked; pending capture cleared.

---

## 23. Exit lifecycle

`photographyRuntime.exit()` + `missionRuntimeCoordinator.exitAndTeardown()`:
detach listener, clear pending, revoke URLs, unload location, reset session
facade, return to Expeditions.
...
| `npm run test:ci` | 606 | **681** |
| `tsc` app/spec | clean | clean |
| `npm run build` | — | **successful** |
| Node | — | `v22.22.3` (pinned) |

Focused new coverage includes score allocation, boundary grace (60/120/30 Hz),
angular-velocity mapping, stability window, objective runtime, results facade,
HUD/results components, null presentation adapter, dependency boundaries.

---

## 27. Exact Checkpoint 6 persistence scope

Checkpoint 6 may add:

- Persisted mission attempt / result records
- Personal-best and medal evaluation from history
- Optional cloud sync of scores (not presentation blobs by default)

Checkpoint 6 must **not** change authoritative capture timing, scoring math,
or score allocation contracts without a version bump.
...
```mermaid
flowchart TB
  Obs[Authoritative observation] --> Stab[Stability window]
  Obs --> Bound[Boundary runtime]
  Obs --> Crash{Crashed?}
  Crash -->|yes| FailCrash[Fail AIRCRAFT_CRASHED]
  Bound -->|expired| FailOOB[Fail OUT_OF_BOUNDS]
  Obs --> Pending{Pending shutter?}
  Pending -->|yes| Evidence[PhotoEvidenceBuilder]
  Evidence --> Score[evaluatePhotoCapture]
  Score -->|pass| Accept[acceptObjective + presentation]
  Score -->|fail| FailAttempt[recordFailedAttempt]
```

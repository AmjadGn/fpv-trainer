...
`PhotoEvidenceBuilder` fills schema **2.0.0** `PhotoCaptureEvidence` from
explicit build-input context (no Angular singleton discovery):

- Identity: schema version, capture id (`session:g{gen}:objective:attempt`),
  mission/location/session generations and versions, objective version,
  scoring-policy version, authoritative tick + mission elapsed ticks
- Aircraft: id, source type, definition/physics/runtime versions, pose,
  linear/body angular velocity, altitude, armed, crashed, optional position zone
- Camera: canonical rig id/version/strategy, world/local mount poses,
  projection (FOV/aspect/near/far/version), mode, cosmetics excluded
- Spatial: LOS / obstruction ratios, optional primary distance
- Subjects: deterministic order; **bounds-derived** rectangle/coverage/
  intersection; scoring-anchor centering/distance/angle/side; exact visibility
  sample counts + visibility/obstruction ratios
- Stability: authoritative linear/angular speeds, stable/required ticks,
  `isStable`

Deeply immutable, JSON-serializable, free of Angular signals / Three / Rapier /
...
## Review corrections (pre-merge)

The initial Checkpoint 5 PR implementation incorrectly derived the framing
rectangle from `visibilitySamplePoints` via `projectSubjectSamplePoints`. That
was corrected before merge. Framing and coverage now use authored
`subjectBounds`; LOS continues to use `visibilitySamplePoints`.

### 1. Subject bounds versus visibility sample authority

| Field | Authority |
| --- | --- |
| `subjectBounds` | Projected subject size, screen rectangle, coverage, frame intersection |
| `visibilitySamplePoints` | Deterministic Rapier LOS / visible sample count / obstruction |
| `scoringAnchor` | Projected anchor, centering error, distance, viewing angle/side |

### 2. Supported bounds projection shapes

Pure helper `getSubjectBoundsProjectionPoints` / `projectSubjectBounds` in
`@fpv/photography-domain` (no Angular / Three / Rapier):

- **AABB** — all eight corners
- **OBB** — eight oriented corners from authored pose
- **Sphere** — eight corners of the enclosing axis-aligned cube (deterministic; no random surface samples)
- **Polygon prism** — lower and upper vertices in authored order

Unsupported/unsafe geometry returns a structured evidence-construction failure
(never falls back to visibility points).

### 3. Near-plane policy

- Non-finite geometry → construction failure
- Camera inside subject bounds → construction failure (no arbitrary huge rectangles)
- All bounds points behind camera / none within clip range → `screenRectangle: null`
- Partial near-plane crossing → only `withinClipRange` points contribute
- Non-finite projected screen coordinates → construction failure

### 4. Evidence schema v2

`EVIDENCE_SCHEMA_VERSION = '2.0.0'`. Previously optional/absent mission and
location context is now required. Unreleased v1 evidence is not migrated.
...
```text
evidenceId, schemaVersion,
missionId, missionVersion, missionSessionId, sessionGeneration,
objectiveId, objectiveVersion, attemptNumber,
locationId, locationVersion, locationGeneration,
capturedAtTick, missionElapsedTicks,
scoringPolicyVersion
```

Opaque strings/numbers only — photography-domain does not import mission or
location domain packages. No `Date`; no wall-clock scoring authority.

### 6. Aircraft and camera provenance

Aircraft snapshot records `aircraftId`, `aircraftSourceType`,
`definitionVersion`, `physicsProfileVersion`, `runtimeCompatibilityVersion`,
pose, velocities, altitude, armed/crashed, optional `positionZoneId`.

Camera snapshot records `rigId`, `rigVersion`, `resolutionStrategy`,
world/local mount poses, projection (FOV/aspect/near/far/model version),
optional tilt, mode, `cosmeticEffectsExcluded = true`, optional
`templateDerivedCamera`. Canonical resolved rig only — never the cosmetic
Three.js camera.

### 7. Exact visibility sample counts

`MissionVisibilitySampleResult` returns `visibleSampleCount` and
`totalSampleCount`. Subject observations store those exact integers; counts
are never reconstructed by rounding an aggregate ratio.

### 8. Deep immutability

`createPhotoCaptureEvidence` deep-clones then deep-freezes the evidence tree,
including nested pose/vector/rectangle objects. Shallow freeze alone is not
sufficient.

### 9. Retry-safe capture IDs

```text
buildCaptureId(sessionId, sessionGeneration, objectiveId, attemptNumber)
→ `${sessionId}:g${sessionGeneration}:${objectiveId}:${attemptNumber}`
```

Retry with a new session generation yields different IDs; attempt semantics and
infrastructure non-attempt policy are unchanged.

### 10. Locked chase-camera policy (v1.3.0)

- Chase remains switchable during the mission
- Capture remains blocked outside FPV
- Framing guide hides when FPV capture is unavailable
- HUD explains that the pilot must return to FPV to capture
- Camera is not locked completely to FPV

### 11. Locked Checkpoint 6 IndexedDB strategy

Checkpoint 6 will use IndexedDB database `fpv-missions-v1` (not localStorage,
not a backend for the first durable implementation). Planned durable data:
mission result records, objective breakdowns, completion flags, personal-best
comparison, latest-result metadata. Images: retain only accepted presentation
images associated with the personal-best result, max one image per objective
(Coastal Ruins Survey max three). Image or quota failure must not invalidate
the mission result. **Not implemented in this correction.**
